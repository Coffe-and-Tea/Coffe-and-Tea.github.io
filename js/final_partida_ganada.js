// Screen fade controller for WIN condition
(function() {
    // Verificar si la aplicación PIXI está disponible (igual que en el fade de Game Over)
    const pixiApp = typeof app !== 'undefined' ? app : (typeof window.app !== 'undefined' ? window.app : null);

    if (!pixiApp) {
        console.error("PIXI Application 'app' not found. Win screen fade cannot be initialized.");
        return;
    }
    
    console.log("[WIN CONTROLLER] Script initialized and PIXI app found.");

    // --- Variables de FADE ---
    const overlay = new PIXI.Graphics();
    overlay.alpha = 0;
    overlay.position.set(0, 0); 
    // Asegúrate de añadir el overlay si no lo tienes ya en el escenario
    ...
    let fadingIn = false;
    let gameWon = false; // Flag para asegurar que la lógica de victoria solo corra una vez
    
    // --- Estilos de la UI (Reutilizando los estilos de Game Over) ---
    const WIN_TEXT_STYLE = {
        fontFamily: "Special Elite",
        fontSize: 36,
        fill: 0x00ff00, // Verde para la victoria
        stroke: 0x000000, 
        strokeThickness: 4, 
        align: 'center'
    };
    ...
    function createWinUI() {
        ...
        button.on('pointerdown', () => {
            console.log("[WIN CONTROLLER] Botón JUGAR DE NUEVO presionado. Recargando página...");
    ...
    pixiApp.ticker.add(() => {
        ...
        if (fadingIn) {
            ...
            if (overlayAlpha >= targetAlpha) {
                ...
                fadingIn = false; 
                console.log(`[WIN CONTROLLER] Fade-in finished at ${targetAlpha * 100}%.`);
                ...
                if (winContainer) {
                    pixiApp.stage.setChildIndex(winContainer, pixiApp.stage.children.length - 1);
                }
            }
            overlay.alpha = overlayAlpha;
        ...
        const allBlackSheepFound = (typeof blackSheepCount !== 'undefined' && blackSheepCount <= 0);

        if (!gameWon && timerFinished && allBlackSheepFound) {
            gameWon = true; // Prevenir múltiples llamadas
            console.log("[WIN CONTROLLER] ¡Condición de Victoria alcanzada! Iniciando Fade.");
